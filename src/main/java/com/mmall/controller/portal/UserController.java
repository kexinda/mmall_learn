...
public class UserController {
    ...
    public ServerResponse<User> getUserInfo(HttpSession session){
        ...
        return ServerResponse.createByErrorMessage("User not login, cannot get current user info");
    }
    ...
    public ServerResponse<User> get_information(HttpSession session) {
        User current_user = (User) session.getAttribute(Const.CURRENT_USER);
        if (current_user == null) {
            return ServerResponse.createByErrorCodeMessage(ResponseCode.NEED_LOGIN.GetCode(), "user didn't login, status = 10");
        }
